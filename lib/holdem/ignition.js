...
class HoldemIgnitionParser extends HandHistoryParser {
  ...
  _readSummarySinglePot(line, lineno) {
    ...
    action.rake = amount - this._potWhitoutRake 
    // Fix precision
    action.rake = Math.round(action.rake * 100) / 100
...
HoldemIgnitionParser.prototype._postRx              = /^([^:]+): (Ante chip|Small blind|Big blind|Posts chip|Posts dead chip) [$|€]?([^ ]+)$/i // Big Blind : Big blind 20
...
HoldemIgnitionParser.prototype._actionRx            = /^([^:]+) : (raises|All-in\(raise\)|bets|All-in\(bet\)|call|calls|All-in|checks|folds(?:\(timeout\))?) ?[$|€]?([^ ]+)?(?: to [$|€]?([^ ]+))?(.+all-in)?$/i
...
exports.parse = function parse(lines, opts) {
  ...
  if (opts && 'hero' in opts) hero = opts.hero
  if (opts && 'prefix' in opts) prefix = opts.prefix
