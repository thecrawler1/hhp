...
function actionType(s, firstAllin) {
  ...
  if (/all-in/i.test(s)) return firstAllin ? 'bet' : 'call'
  return s
}
...
class HandHistoryParser {
  constructor(lines, opts) {
    ...
    this._firstAllin      = true
    this._potWhitoutRake  = 0.0
  ...
  _postType(s) {
    ...
    return (lower === 'ante' || lower === 'ante chip')  ? 'ante'
          : lower === 'big blind'                       ? 'bb'
          : lower === 'small blind'                     ? 'sb'
          : lower === 'posts chip'                      ? 'post'
          : lower === 'posts dead chip'                 ? 'post'
          : 'unknown'
  }
  ...
  _readStreet(line, lineno) {
    ...
    if (match[1] === 'FLOP') {
      this._sawFlop = this._firstAllin = true
    }
    if (match[1] === 'TURN') {
      this._sawTurn = this._firstAllin = true
      this.hand.board.card4 = this.hand.board.card5
      this.hand.board.card5 = undefined
    }
    if (match[1] === 'RIVER') this._sawRiver = this._firstAllin = true
    return true
  }
  ...
  _readAction(line, lineno) {
    ...
    const type = actionType(match[2], this._firstAllin)
    const action = {
        player  : safeTrim(match[1])
      , type    : type
    ...
    if (type === 'raise' || type === 'call' || type === 'bet') {
      action.allin = !!match[5] || /all-in/i.test(match[2])
    }
    if (action.allin) {
      this._firstAllin = false
    }
